...
- **Single-Subject Focus vs. Multi-Subject Conditioning Degradation**: Single-character generations consistently achieve high scores (**7.5 - 9.0 / 10**), whereas multi-character generations featuring two children together suffer from conditioning payload splitting, averaging **5.5 / 10** due to feature blending and facial smoothing.
- **Contextual Anchors Rescue Multi-Character Likeness**: High-context environments with iconic costuming (e.g. `both_starwars_world.png` with Jedi robes and lightsabers) rescued multi-character likeness up to **7.5 / 10**!
