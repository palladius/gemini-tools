...
1. **Python Execution**: Always use `#!/usr/bin/env -S uv run` with inline script dependencies. Do not use global `pip` or virtualenv activation.
...
4. **Justfile**: Maintain `Justfile` tasks. The default target must always be `list -> just -l`.
